...
namespace Swordfox\CDNRewritePlus;
...
class CDNMiddleware implements HTTPMiddleware
{
    ...
    private function rewriteTags(&$body)
    {
        ...
        $assetprefix = $this->config()->get('assetprefix');
        $rewrites = $this->config()->get('rewrites');
        $rewritelinks = $this->config()->get('rewritelinks');
        $rewritebgattr = $this->config()->get('rewritebgattr');

        foreach ($rewrites as $rewrite) {
            $cleanrewrite = trim($rewrite, '/');

            $search = [
                'src="' . $subDir . $cleanrewrite . '/',
                'src="/' . $subDir . $cleanrewrite . '/',
                'src=\"/' . $subDir . $cleanrewrite . '/',
                Director::absoluteBaseURL() . $cleanrewrite . '/'
            ];

            if ($cleanrewrite == 'assets' and $assetprefix) {
                $cleanreplace = $assetprefix . $cleanrewrite;
            } else {
                $cleanreplace = $cleanrewrite;
            }

            $replace = [
                'src="' . $cdn . '/' . $subDir  . $cleanreplace . '/',
                'src="' . $cdn . '/' . $subDir . $cleanreplace . '/',
                'src=\"' . $cdn . '/' . $subDir  . $cleanreplace . '/',
                $cdn . '/' . $subDir . $cleanreplace . '/'
            ];

            if (($cleanrewrite == 'assets' and $rewritelinks) or $cleanrewrite != 'assets') {
                array_push($search, 'href="/' . $subDir . $cleanrewrite . '/');
                array_push($replace, 'href="' . $cdn . '/' . $subDir . $cleanreplace . '/');
            }

            if ($rewritebgattr) {
                array_push($search, 'background-image: url(/' . $subDir . $cleanrewrite . '/');
                array_push($replace, 'background-image: url(' . $cdn . '/' . $subDir . $cleanrewrite . '/');
            }
